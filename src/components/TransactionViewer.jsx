...
export default function TransactionViewer({ user, accountSummary, onLogout }) {
  ...
  const calculateReceiptCount = () => {
    ...
    if (groupBy === 'none') {
      ...
      // For grouped mode, count unique business purposes from groups with selected transactions
  ...
  const handleGeneratePDF = async () => {
    ...
    if (groupBy === 'none') {
      ...
      // For grouped mode, organize by business purpose
      Object.entries(grouped).forEach(([groupKey, groupTransactions]) => {
        ...
        if (selectedInGroup.length > 0) {
          ...
          groupedForPDF[purpose].push(...selectedInGroup);
        }
      });
    }
